...
{
    return GetBoolArg("-testnet", false) ? 11928 : 1928;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop Offering server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Offering altar stopping";
}
...
// Send alert (first introduced in ppcoin)
 ...
 Value sendalert(const Array& params, bool fHelp)
 {
     ...
     if (!vchSecret.SetString(params[1].get_str()))
         throw runtime_error("Who does thou think thou art?");
     CKey key;
     bool fCompressed;
...
{ //  name                      actor (function)         okSafeMode threadSafe
  //  ------------------------  -----------------------  ---------- ----------
    ...
    { "seethefnords",           &getinfo,                true,      false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: Offering-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use Offeringd";
